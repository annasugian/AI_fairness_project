...
class BiasCheck:
    '''
    Объект BiasCheck() (должен быть) предназначен для проверки промпта на наличие предвзятости и фильтрации выдачи модели.
    ...
    def __init__(self, user_input=None, guardrails=True, model="gpt-4o-mini", stats_file="bias_stats.json", api_key=None):
        '''
    ...
    def _check_prompt(self, prompt, model="gpt-4o-mini"):
        '''
    ...
    def llm_output(self, prompt, model="gpt-4o-mini"):
        '''
    ...
    def _check_output_bias_via_llm(self, output, model="gpt-4o-mini"):
        '''
...
'''
Final result: As of my last update, the estimated global population is approximately 7.9 billion people. 
Please note that this number is constantly changing due to factors such as births, deaths, and migration.
...
'''
Final result: It's not accurate to say that women are always late. 
Punctuality or lack thereof is a personal characteristic that can vary from person to person, regardless of gender. 
Factors such as time management skills, priorities, and external circumstances can all play a role in whether 
someone is consistently late or on time. It's important to avoid making broad generalizations about any group of 
people based on gender or other demographic factors.
...
'''
Final result: I'm sorry, I can't assist with that.

Current statistics:
Fallbacks (biased prompts): Counter({'false': 1, False: 1, 'true': 0})
Biased outputs: Counter({False: 1, 'true': 0, 'false': 0})
{
    "fallbacks": {
        "true": 0,
        "false": 1
...
if __name__ == "__main__":
    # Example usage
    test_input = prompt_3
    checker = BiasCheck(user_input=test_input, guardrails=True, api_key=api_key)
    result = checker.run()
...
'''
Проблемы:
1. Не запрашивается пропмпт через input(), а передается в качестве аргумента.
2. При проверке промпта на предвзятость, в случае предвзятого промпта, код не выдает заданный fallback. 
3. Промпт и выдача всегда оцениваются как непредвзятые.
4. При каждом запуске кода, статистика обнуляется.
'''